function [r,face,r2R,R]=read_3D_from_OBJ(filename)
% read 3D points from an OBJ file
% We have not previously done this; because we assumed
% 3D points, if necessary, came from another source
...
while ~feof(fid)
    ...
    if length(tline)>2
        if min(tline(1:2)=='v ')
            ...
            fprintf(fid_v,'%s\n',tline(3:end));
        elseif min(tline(1:2)=='vt')
            fprintf(fid_vt,'%s\n',tline(4:end));
        elseif min(tline(1:2)=='f ')
            i=find(tline=='/');
            tline(i)=' ';
            fprintf(fid_f,'%s\n',tline(3:end));
        end
